...
public struct AgentIntegrationInstaller {
    ...
    /// Where the pane-team guide (`ControlCommandLineParser.paneTeamGuide`,
    /// the same text `mytty-ctl guide` prints) is written as a standalone
    /// Markdown file -- one canonical, dev-and-release-shared copy next to
    /// the shared `mytty-ctl` binary (see `AppDelegate.controlExecutableDestination`),
    /// so a pointer written by either build variant resolves to the same
    /// path. Kept as a stored property, not derived ad hoc, so the pointer
    /// text below and any UI that displays this path can't drift from what
    /// `installGuideMarkdown` actually writes.
    public let installedGuideMarkdown: URL
    ...
    public init(
        ...
        installedGuideMarkdown = applicationSupportDirectory
            .appendingPathComponent("mytty-ctl.md", isDirectory: false)
    }

    /// Whether the on-disk guide Markdown file matches the binary's current
    /// `ControlCommandLineParser.paneTeamGuide` text. English-only, like the
    /// guide itself -- this file is never routed through
    /// `PaneTeamPointerLanguage`.
    public func guideMarkdownStatus() -> AgentIntegrationStatus {
        guard fileManager.fileExists(atPath: installedGuideMarkdown.path)
        else { return .notInstalled }
        guard let data = try? Data(contentsOf: installedGuideMarkdown) else {
            return .needsRepair
        }
        return data == guideMarkdownData() ? .installed : .needsRepair
    }

    /// Writes (or rewrites) the guide Markdown file so it matches the
    /// running binary's `mytty-ctl guide` output. Called unconditionally on
    /// every launch (see `AppDelegate.launchApplication`) so the file never
    /// drifts behind an app update; callers treat failures as best-effort
    /// and must not let them block startup.
    public func installGuideMarkdown() throws {
        try writeAtomically(
            guideMarkdownData(),
            to: installedGuideMarkdown,
            defaultMode: 0o644
        )
    }

    private func guideMarkdownData() -> Data {
        Data(ControlCommandLineParser.paneTeamGuide.utf8)
    }
    ...
    private func paneTeamSkillData(
        ...
        let guidePath = installedGuideMarkdown.path
        switch language {
        case .english:
            return Data(
                """
                ...
                When asked to coordinate work across multiple panes or run
                other AI agents as sub-agents, first read \(guidePath) -- the
                full operating manual. The same content also prints from
                "$MYTTY_CTL_BIN" guide.

                Generated by Mytty. Safe to delete; recreated by turning
                "Teach agents about Mytty orchestration" back on in Mytty's
                settings.
                """.utf8
            )
        case .japanese:
            return Data(
                ...
                複数ペインにまたがる作業や、別の AI エージェントをサブ
                エージェントとして動かす依頼を受けたときは、まず
                \(guidePath) を読む。手順の全文が書かれている。同じ内容は
                `"$MYTTY_CTL_BIN" guide` でも出せる。

                Mytty が生成したファイル。削除しても構わない。Mytty の設定で
                「Agent に Mytty オーケストレーションの使い方を教える」を
                再度オンにすれば作り直される。
                """.utf8
            )
        }
    ...
    private func paneTeamBlockBody(
        ...
        case .japanese:
            ...
            mytty-ctl はローカルの Unix ソケットに接続するため、Codex 自身のサンドボックスの外で実行してください。

            Mytty が生成したブロック。削除しても構わない。Mytty の設定で
            「Agent に Mytty オーケストレーションの使い方を教える」を再度
            オンにすればこのブロックは作り直される。
            \(Self.paneTeamBlockEnd)
            """
        }
