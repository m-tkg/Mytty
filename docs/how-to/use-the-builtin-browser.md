# Use the built-in browser

Mytty can open a browser pane inside a pane. It's handy for viewing local documents or a preview server.

## Open a local file

Choose **Open HTML File** to display the HTML in the active pane.

## Follow a link from a terminal

Hold Command in a terminal to underline links that can be opened in the browser. Clicking one brings up a menu to open it in an external browser, open it in another pane, and more.
