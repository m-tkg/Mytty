...
CLI の PATH インストールとエージェントへの案内トグルは、設定 >
Orchestration に集約されている。CLI(`mytty-ctl` へのシンボリックリンク)
のインストール状態、Claude Code(`~/.claude/skills/mytty-panes/SKILL.md`)
と Codex(`~/.codex/AGENTS.md`)それぞれの案内ファイルの状態、実際に
書き込まれる文面のプレビュー(押しても書き込みは行われない)、現在の設定に
合わせた呼び出し方の例が1画面にまとまっている。案内トグルをオンにしておくと、
対応するエージェントは複数ペインでの調整を頼まれたときに自分から
`mytty-ctl guide` を実行するようになる。
