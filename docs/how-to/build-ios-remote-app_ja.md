...
XcodeGen で Xcode project を再生成し、iOS Simulator 向けにビルドします。実機向けには `make ios-device` を使いますが、その前に署名の設定が必要です(後述)。
...
署名の既定値(`DEVELOPMENT_TEAM` と `MYTTY_BUNDLE_ID`)は `ios/MyttyRemote/Config/Signing.xcconfig` にあります。このファイルは直接編集せず、`Config/Local.xcconfig` を作って上書きします。
