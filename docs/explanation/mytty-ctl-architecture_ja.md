...
## status の自己申告がイベントパイプラインを通らない理由

`mytty-ctl status` は一見すると単なる新しいエージェントイベントに見えますが、意図的にそうしていません。hook イベントは構造化されたライフサイクルの事実です: SQLite に永続化され、純粋な reducer で `AgentRun` に再構成され、リトライに対して冪等です。自己申告の「running tests」はそのどの性質も持ちません — run の状態を変えず、自分が説明している run より長生きしてはならず(アプリ再起動をまたぐなど論外)、発信者も per-surface capability を持つ hook ではなく worker 自身のシェルコマンドです。パイプラインに通すなら、全 reducer の網羅的 switch が無視しなければならない新 event kind と、あってはならない永続化を抱え込むことになります。

そこで申告はパイプラインの脇に置いています: `AttentionCenter` 上のメモリのみの `paneStatusNotes` 辞書に、制御ソケット経由(その同一ユーザー信頼モデルはペインへの入力送信などずっと強い操作を既に許しています)で書き込み、ステータスバー・`list`・`agent` の job スナップショットが読み出し、失効はパイプライン自身のイベントに任せます — run の開始・終了がそのペインの申告をクリアします。イベントストリームとの接点はこの失効だけで、それ以外は一切関与しません。
