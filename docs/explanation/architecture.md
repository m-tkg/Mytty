...
- **`MyTTYRemoteKit`** is the remote protocol and every client's half of it: pairing, the secure channel, the message codec, the `RemoteClient` session, Bonjour discovery, and the screen renderer. It is shared rather than iOS-specific because a Mac is a client too — a remote pane mirrors another Mac's terminal through exactly this code. Nothing here may touch UIKit or AppKit, which is what lets both platforms use one implementation of the protocol instead of two that drift.

A remote pane is worth calling out as a design choice. It is its own `SplitNode` case rather than a terminal with an unusual command, because it has no local process, no working directory, and no Ghostty surface: everything it shows arrived as styled runs over the wire. Making it a separate kind is what lets the compiler point at every place that assumed "pane means local terminal" — agent status polling, repository status, `mytty-ctl` addressing — so each one skips it deliberately instead of silently reporting a neighbouring pane's state. Its connections are owned app-wide by `RemotePaneConnectionCoordinator` rather than per window, so two panes onto the same Mac share one session and a pane dragged between windows keeps mirroring.
...
Agent state is derived from versioned, idempotent events delivered by explicit hooks or by libghostty's own terminal protocol support (see `docs/reference/agent-event-protocol.md` for the wire format). Human-readable terminal output is never parsed to infer state. This rules out the tempting shortcut of regexing an agent's CLI output for phrases like "Waiting for approval": that text changes across provider versions and locales without notice, and a false read would either spam the Attention Inbox or silently drop a real approval request. Events are the only source that can carry a stable schema version. Where a provider's hook integration isn't installed, run state is not left `unknown`: Mytty estimates the lifecycle natively from the pane's polled foreground process, OSC 133 command-end, and (for Claude Code, Codex, and Cursor) that provider's own transcript or store. The attention states, `waiting-input`/`waiting-approval`, are the one exception -- native estimation deliberately never synthesizes them, since a misread there could put a false actionable item in the Attention Inbox, so those stay unreported until the provider's real hooks are installed.
...
The same poll also feeds `NativeAgentRunEstimator`, which uses the detected foreground process plus OSC 133's command-finished signal to derive run lifecycle for a provider whose hook integration isn't installed, and for Claude Code, Codex, and Cursor further splits that into a run per prompt turn by reading the same transcript/`store.db` data the session inspector above already parses. This is what keeps run state observable even before a hook is ever installed; see [Agent event protocol](../reference/agent-event-protocol.md#native-run-estimation) for the full mechanism.
