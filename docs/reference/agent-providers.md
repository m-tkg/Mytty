...
A wrapper that stays on the machine — a shell script that launches the agent as a child rather than `exec`ing it — needs no hint: when the pane's foreground process is a shell that Mytty does not recognize as an agent, it looks at that shell's descendants (two levels, a handful of processes) and reports the agent it finds there, along with the process running it. Session inspection, the resume flags and the displayed name all follow that process rather than the wrapper.

For a wrapper Mytty cannot see through — `ssh` into a remote box, `docker exec` into a container — export `MYTTY_AGENT=<provider>` in the wrapper's environment before launching it. When path/argv detection identifies no provider, Mytty falls back to reading this variable from the wrapper process's exec-time environment. The value must be one of the `AgentProvider` identifiers — `codex`, `claude-code`, `opencode`, `antigravity`, `cursor` — compared after trimming whitespace and lowercasing; anything else is ignored. A hint never overrides real detection, and it only affects provider identification (the status bar label and provider-scoped lookups). Hook events still have to reach Mytty's event socket, which a remote host or container cannot do, so panes detected via the hint typically show the provider without run states.
...
| `succeeded` | `Stop` | `Stop` (no in-flight background tasks) | `session.idle` | idle `Stop` | `stop` with `completed`, or no `status` field at all |
...
Claude Code's `Stop` fires at the end of the main turn even while background subagents it spawned are still running -- as of v2.1.145 its `Stop` payload carries a `background_tasks` array listing every task still in flight. While that array has a `subagent` or `workflow` entry that isn't yet reported finished, Mytty maps `Stop` to `running` instead of `succeeded`, so sleep prevention and Attention keep treating the session as busy. `shell`, `monitor`, `teammate`, `cloud session`, and `MCP task` entries don't count toward this -- a long-lived `shell` task (a dev server, `tail -f`) shouldn't keep the Mac awake indefinitely. The turn that resumes once the background work finishes carries a new `prompt_id`, so its own `Stop` (with no in-flight entries left) reports the real completion on a new run; the run left behind by the background wait is closed with a synthetic `mytty.runSuperseded` event the next time a `started` event lands on the same pane. Claude Code's `idle_prompt` notification (~60s after `Stop`) is never mapped to `input-requested` for the same reason: fired mid-background-wait it would otherwise misread as a request for user input. `agent_needs_input` and `elicitation_dialog` still map normally.
