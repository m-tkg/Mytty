...
Cursor にも permission prompt 専用の hook は無いが、mytty はあらゆる tool call の前に発火する `preToolUse` からそれを推定します — シェルコマンドに限らず、ファイルの編集や削除も承認プロンプトの対象になります。`preToolUse` はその tool call の `tool_use_id` を key に10秒のタイマーを開始し、同じ `tool_use_id` を持つ `postToolUse` / `postToolUseFailure`、またはその run の `stop` が先に届けばタイマーはキャンセルされます。tool call は同時に複数走ることがあります — 実機で、異なる2つのツールの `preToolUse` が連続して届き、どちらの `postToolUse` もまだ来ていない状態が観測されています — そのため pending タイマーは run 単位ではなく `tool_use_id` 単位で追跡します。run 単位にすると、同じ run 内の別の tool call が解決した瞬間に、まだ pending な tool call を見失ってしまうためです。時間内に何も届かなければ、mytty 自身が `approval-requested` event を合成します — pending 状態は `CursorApprovalPendingTracker` が保持し、タイマーは `CursorApprovalCoordinator` (`Sources/MyTTYApp/CursorApprovalCoordinator.swift`) が持ち、event 自体は `AgentHookEventAdapter.pendingApprovalEvent` が組み立てます。その後一致する `postToolUse` または `postToolUseFailure` が届けば、実際の approval が解決する場合と同じ経路で run は `running` に戻るため、解決処理を別途書く必要はありません。10秒以内に終わる自動承認 tool call はそもそも発火しません。自動承認だが実行が長引いた tool call は、一時的に Attention に `approval-requested` として出た後、その `postToolUse` が届いた時点で自動的に解決します。

mytty はもう Cursor の `beforeShellExecution` / `afterShellExecution` hook にハンドラを導入せず、承認待ちの検知にも使いません: この2つはシェルコマンドしか挟まないため、非シェルの tool call(実機で観測されたファイル削除の承認待ちなど)が承認プロンプトで詰まってもどちらの hook も発火せず、これに基づく遅延推定は取りこぼしていました。この2つの hook のマッピング自体は、手動でこれらを導入した人のために引き続き認識されますが、設定画面から Cursor 連携をインストール・修復すると、今後は `preToolUse` が書き込まれます。
