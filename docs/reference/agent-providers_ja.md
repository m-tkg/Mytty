...
**Agent に Mytty オーケストレーションの使い方を教える**(設定 > Orchestration、
デフォルトオン)は、
グローバルなポインタの置き場所が確認できている2つの provider に対して、
上記とは別のファイルに短い参照を書き込む。
...
実際の使い方の本文(環境変数、split/send/wait/read の流れ、provider
ごとの起動フラグ)は埋め込まれない。代わりに Mytty が起動のたびに
`~/Library/Application Support/mytty/mytty-ctl.md` へ書き出す
(`ControlCommandLineParser.paneTeamGuide`、`mytty-ctl guide` の出力と
同一の英語のみのテキスト)。SKILL.md と AGENTS.md の管理ブロックは
どちらもこのファイルの絶対パスを指すだけで、中身を複製しない。そのため
Mytty がアップデートされて使い方が変わっても、書き直しが必要なのは
`mytty-ctl.md` 側だけで、2つの参照ファイルは(パスが変わらない限り)
書き直し不要のまま正確であり続ける。Cursor・OpenCode・Antigravity は
グローバルな指示の置き場所がまだ確認できていないため対象外。

参照文自体の文面は 設定 > General の言語設定(英語・日本語)に従う。
`mytty-ctl.md` の本文(`paneTeamGuide`)は英語のみで、言語設定の影響を
受けない。`AppLanguage.systemDefault` の解決はアプリ層が行い、
MyTTYCore には解決済みの言語だけが渡る。言語を切り替えると、次に
アプリケーション設定が変わったタイミングで既存のファイルが新しい言語の
参照文に書き直される
(`AgentIntegrationSettingsModel.repairInstalledIntegrations(language:)`)。
管理ブロックの `<!-- mytty:pane-team:begin -->` / `:end` マーカーと
SKILL.md の `name: mytty-panes` は言語に関係なく共通。
