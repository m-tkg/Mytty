...
残りコンテキストが一定の割合を下回ると、コンテキストメーターは赤くなります。通知を出さずに、残量が尽きかけているペインを見分けられます。**設定 > Agents > コンテキスト残量が少ないときに警告** で、この警告を切ったり割合を変えたり(既定は 30%)できます。判定にはメーターが表示している丸めた後の割合を使います。
