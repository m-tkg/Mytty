...
| `succeeded` | `Stop` | `Stop`(background task が in-flight でない場合) | `session.idle` | idle 時の `Stop` | `completed` を伴う `stop`、または `status` フィールド自体が無い `stop` |
...
Claude Code の `Stop` は、自分が spawn したバックグラウンドの subagent がまだ走っている状態でも、メインターンの終わりに発火します。v2.1.145 以降、その `Stop` のペイロードには in-flight のタスクを列挙する `background_tasks` 配列が含まれます。この配列に `subagent` または `workflow` の、まだ完了報告されていないエントリが残っている間、mytty は `Stop` を `succeeded` ではなく `running` にマッピングします。これにより sleep prevention と Attention は、そのセッションをまだ稼働中として扱い続けます。`shell`、`monitor`、`teammate`、`cloud session`、`MCP task` はこの判定に含めません -- 長時間動く `shell` タスク(dev server や `tail -f` など)のせいで Mac がいつまでも sleep しなくなるのを避けるためです。バックグラウンドの作業が終わって再開されたターンは新しい `prompt_id` を持つため、その `Stop`(in-flight のエントリが残っていない状態)が新しい run 上で本当の完了を報告します。バックグラウンド待ちで取り残された古い run は、同じペインに次の `started` event が届いたタイミングで、`mytty.runSuperseded` という合成 event によって閉じられます。同じ理由で、Claude Code の `idle_prompt` notification(`Stop` の約60秒後に発火)は `input-requested` にマッピングしません -- バックグラウンド待ちの最中に届くと、ユーザーへの入力要求と誤読してしまうためです。`agent_needs_input` と `elicitation_dialog` は従来どおりマッピングされます。
...
2 つの status bar は同じペイン単位の状態を読みます。ウィンドウ下部のバーはフォーカス中のペインを、ペイン自身のバー（タブが分割されているときに出ます）はそのペインを表します。ペインのバーではセッション識別子を先頭 8 文字に短縮します。
...
コンテキストメーターと、その隣に並ぶ使用量リミットのメーターは、既定では残量をグラフにします。**設定 > Agents > メーターの表示** で、どちらも使用量の表示に切り替えられます。切り替わるのは表示だけで、Mytty が読み取っているのはどちらの場合も残量です。コンテキスト残量の警告も、同じしきい値と残量の割合を比べ続けます。
