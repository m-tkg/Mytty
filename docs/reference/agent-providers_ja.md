...
書き込まれる文面は 設定 > General の言語設定(英語・日本語)に従う。
`AppLanguage.systemDefault` の解決はアプリ層が行い、MyTTYCore には
解決済みの言語だけが渡る。言語を切り替えると、次にアプリケーション設定
が変わったタイミングで既存のファイルが新しい言語の文面に書き直される
