...
- **タブとペイン:** 4方向への分割、挿入位置を表示するドラッグ並べ替え、
  ペインズーム、均等割り当て、全ペインスイッチャー。分割中のタブでは、
  非アクティブ側を暗くするのに加えてアクティブなペインを枠線で囲む
  (色と太さは設定可能)。タブにはサイドバーで番号が振られ、Command-1〜9
  で直接ジャンプできる。新規タブは末尾か、現在のタブの直後かを選べる
  (Settings)。
